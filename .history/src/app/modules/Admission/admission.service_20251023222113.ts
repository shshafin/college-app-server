...
// UPDATE admission
// const updateAdmission = async (id: string, data: Partial<IAdmission>): Promise<IAdmission | null> => {
//   try {
//     const admission = await Admission.findByIdAndUpdate(id, data, { new: true });
//     if (!admission) {
//       throw new AppError(404, `Admission with id ${id} not found`, 'Admission not found');
//     }
//     return admission;
//   } catch (error: any) {
//     if (error instanceof AppError) throw error;
//     throw new AppError(500, error.message || 'Error updating admission', 'Failed to update admission');
//   }
// };
