...
const searchCollegesByName = async (name: string): Promise<ICollege[]> => {
  try {
    return await College.find({ collegeName: { $regex: name, $options: 'i' } });
  } catch (error: any) {
    throw new AppError(
      500,
      error.message || 'Error searching colleges by name',
